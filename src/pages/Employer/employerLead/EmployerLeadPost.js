...
import { MyModal, capitalizeWords } from "../../../utility";
...
const EmployerLeadPost = ({ reloeadpage, oncloseLead, mobileNumber }) => {
  // const Navigate = useNavigate();

  const adminID = localStorage.getItem("adminID");
  console.log(adminID, "adminIdadminIdadminIdadminIdadminIdadminId");
  ...
  const [leadData, setLeadData] = useState({
    fromAdminId: { val: adminID, err: false },
  ...
  useEffect(() => {
    ...
    GetEmployerLead(mobileNumber).then((data) => {
      if (data.code === 200) {
        ...
        setLeadData((prev) => ({
          ...prev,
          fromAdmin: { val: assignTo || "", err: false },
          companyName: { val: companyName || "", err: false },
          industry: { val: industry || "", err: false },
          city: { val: city || "", err: false },
          contactPersonName: { val: contactPersonName || "", err: false },
...
export default EmployerLeadPost;
