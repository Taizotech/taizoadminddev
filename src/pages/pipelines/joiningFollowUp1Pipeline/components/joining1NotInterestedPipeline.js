...
import Joining1NotInterestedFilter from "./joining1NotInterestedFilter";

function Joining1NotInterestedPipeline() {
  const [leadData, setLeadData] = useState({
    data: [],
  });

  let filterDetails = useSelector(
    (state) => state.CandidatePipelineDetails.joiningFollowUp1NotInterested
  );
  ...
  function paginationChange(event, page) {
    Dispatch(
      CandidatePipelineActions.setPageChange({
        type: "joiningFollowUp1NotInterested",
        value: page - 1,
      })
    );
  }
...
export default Joining1NotInterestedPipeline;
