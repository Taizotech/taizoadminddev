...
import Joining1ConfirmedFilter from "./joining1ConfirmedFilter";

function Joining1ConfirmedPipeline() {
  const [leadData, setLeadData] = useState({
    data: [],
  });

  let filterDetails = useSelector(
    (state) => state.CandidatePipelineDetails.joinningFollowUp1Confirmed
  );
  ...
  function paginationChange(event, page) {
    Dispatch(
      CandidatePipelineActions.setPageChange({
        type: "joinningFollowUp1Confirmed",
        value: page - 1,
      })
    );
  }
...
export default Joining1ConfirmedPipeline;
