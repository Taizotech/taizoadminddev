...
import JoiningFollowUp2LeadLostFilter from "./joining2LeadLostFilter";

function JoinningFollowUp2LeadLostPipeline() {
  const [leadData, setLeadData] = useState({
    data: [],
  });

  let filterDetails = useSelector(
    (state) => state.CandidatePipelineDetails.joinningFollowUp2LeadLost
  );
  ...
  function paginationChange(event, page) {
    Dispatch(
      CandidatePipelineActions.setPageChange({
        type: "joinningFollowUp2LeadLost",
        value: page - 1,
      })
    );
  }
      ...
      {showFilter && (
        <MyModal>
          <ModalContainer
            ...
            childComponent={
              <>
                <JoiningFollowUp2LeadLostFilter
                  onclose={() => {
                    setShowFilter(false);
                  }}
                />
...
export default JoinningFollowUp2LeadLostPipeline;
