...
import Joining2ConfirmedFilter from "./joining2ConfirmedFilter";

function Joining2ConfirmedPipeline() {
  const [leadData, setLeadData] = useState({
    data: [],
  });

  let filterDetails = useSelector(
    (state) => state.CandidatePipelineDetails.joinningFollowUp2Confirmed
  );
  ...
  function paginationChange(event, page) {
    Dispatch(
      CandidatePipelineActions.setPageChange({
        type: "joinningFollowUp2Confirmed",
        value: page - 1,
      })
    );
  }
...
export default Joining2ConfirmedPipeline;
