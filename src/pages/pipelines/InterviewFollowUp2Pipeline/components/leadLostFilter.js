...
function LeadLostFilter({ onclose }) {
  let filterDetails = useSelector(
    (state) => state.CandidatePipelineDetails.followUp2LeadLost
  );
  ...
  useEffect(() => {
    Dispatch(CandidatePipelineActions.setFollowUp2LeadLostFilter(filterData));
  }, [filterData]);

  function handleSubmit() {
    Dispatch(CandidatePipelineActions.setFollowUp2LeadLostFilter(filterData));
    onclose();
    // console.log(CandidatePipelineActions.setFollowUp2LeadLostFilter(filterData));
  }
