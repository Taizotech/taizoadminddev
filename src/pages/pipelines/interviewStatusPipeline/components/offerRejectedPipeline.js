...
function OfferRejectedPipeline() {
  const [leadData, setLeadData] = useState({
    data: [],
  });

  let filterDetails = useSelector(
    (state) => state.CandidatePipelineDetails.interviewStatusOfferRejected
  );
  ...
  function paginationChange(event, page) {
    Dispatch(
      CandidatePipelineActions.setPageChange({
        type: "interviewStatusOfferRejected",
        value: page - 1,
      })
    );
  }
...
export default OfferRejectedPipeline;
