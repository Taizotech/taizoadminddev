...
import StatusOfferAcceptedFilter from "./offerAcceptedFilter";

function OfferAcceptedPipeline() {
  const [leadData, setLeadData] = useState({
    data: [],
  });

  let filterDetails = useSelector(
    (state) => state.CandidatePipelineDetails.interviewStatusOfferAccepted
  );
  ...
  function paginationChange(event, page) {
    Dispatch(
      CandidatePipelineActions.setPageChange({
        type: "interviewStatusOfferAccepted",
        value: page - 1,
      })
    );
  }
...
export default OfferAcceptedPipeline;
